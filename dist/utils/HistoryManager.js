...
class HistoryManager {
    // 保存辩论会话到历史记录
    static saveDebateSession(session) {
        try {
            // 获取现有历史记录
    ...
    static getFilteredHistory(options = {}) {
        ...
        if (options.search) {
            ...
            history = history.filter(item => item.topic.toLowerCase().includes(searchLower) ||
        ...
        if (options.dateRange) {
            history = history.filter(item => item.createdAt >= options.dateRange.start &&
    ...
    // 获取会话链（相关联的连续讨论）
    static getSessionChain(sessionId) {
        try {
            const history = this.getHistory();
            return history
    ...
    static importHistory(jsonData) {
        try {
            ...
            // 只添加不重复的记录
            const newRecords = data.history.filter((item) => !existingIds.has(item.id));
    ...
    static getRelatedSessions(topic, limit = 5) {
        try {
            const history = this.getHistory();
    ...
    // 保存到文件系统
    static saveToFileSystem(historyItem) {
        try {
            // 确保history文件夹存在
    ...
    // 去重函数
    static removeDuplicates(history) {
        const seen = new Set();
...
HistoryManager.STORAGE_KEY = 'intelliround_history';
HistoryManager.STATS_KEY = 'intelliround_history_stats';
HistoryManager.MAX_HISTORY_ITEMS = 1000; // 最大历史记录数量
HistoryManager.HISTORY_DIR = path.join(process.cwd(), 'history');
