...
exports.AIDiagnosticTool = void 0;
...
class AIDiagnosticTool {
    ...
    static async diagnoseOllama(config, issues, suggestions) {
        try {
            // 检查基础连接
            ...
            try {
                ...
                const hasModel = models.some((model) => model.name === config.model);
                if (!hasModel) {
                    issues.push(`模型 "${config.model}" 未安装`);
                    suggestions.push(`请运行 "ollama pull ${config.model}" 下载模型`);
                    suggestions.push(`可用模型: ${models.map((m) => m.name).join(', ')}`);
                }
            }
            catch (error) {
    ...
    static async diagnoseOpenAI(config, issues, suggestions) {
        try {
            const client = axios_1.default.create({
                baseURL: config.baseUrl,
                timeout: 8000,
            ...
            try {
                ...
                const hasModel = models.some((model) => model.id === config.model);
                if (!hasModel) {
...
exports.AIDiagnosticTool = AIDiagnosticTool;
//# sourceMappingURL=AIDiagnosticTool.js.map