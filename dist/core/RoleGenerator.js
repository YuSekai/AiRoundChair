...
class RoleGenerator {
    ...
    async generateRoles(topic, roleCount = 3) {
        ...
        try {
            ...
            const response = await this.aiModel.generateResponse(roleGenerationPrompt, this.getRoleGenerationSystemPrompt(), timeoutMs // 使用配置的超时
            );
    ...
    buildRoleGenerationPrompt(topic, roleCount) {
...
请确保输出严格按照要求的JSON格式，不要包含任何额外的文本。`;
    ...
    parseRolesFromResponse(response) {
        try {
            // 提取JSON部分
...
exports.RoleGenerator = RoleGenerator;
//# sourceMappingURL=RoleGenerator.js.map