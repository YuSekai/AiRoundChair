...
exports.ParallelRoleGenerator = void 0;
...
class ParallelRoleGenerator {
    ...
    // 新增：分析问题类型和相关专业领域
    async analyzeTopicForRoles(topic) {
        const aiModel = AIModel_1.AIModelFactory.create(this.baseConfig);
...
请确保输出是严格的 JSON 格式。`;
    ...
    async generateRoles(topic, roleCount = 3) {
    ...
    async generateSingleRole(topic, roleIndex, totalRoles, topicAnalysis) {
        // 为每个角色创建独立的AI模型实例
        const aiModel = AIModel_1.AIModelFactory.create(this.baseConfig);
    ...
    async generateSingleRoleWithDelay(topic, roleIndex, totalRoles, delay, topicAnalysis) {
        if (delay > 0) {
    ...
    // 串行生成方法（备用方案）
    async generateRolesSequentially(topic, roleCount, topicAnalysis) {
    ...
    buildSingleRolePrompt(topic, roleIndex, totalRoles, topicAnalysis) {
        // 使用问题分析结果来生成相关专业角色
        let roleInfo;
...
请按照以下JSON格式返回，只返回JSON，不要额外文字：
{
  ...
  "expertise": ["专业领域1", "专业领域2", "专业领域3"]
    ...
    getSingleRoleSystemPrompt(roleIndex, topicAnalysis) {
        let instruction;
...
请确保角色具有独特性和代表性，能够从不同角度分析问题。`;
    ...
    parseSingleRoleFromResponse(response, roleIndex) {
        try {
            // 提取JSON部分
    ...
    getDefaultRole(roleIndex, topic, topicAnalysis) {
        // 如果有问题分析结果，优先使用
        if (topicAnalysis && topicAnalysis.suggestedRoles && topicAnalysis.suggestedRoles[roleIndex - 1]) {
    ...
    getFallbackRoles(topic, topicAnalysis) {
        return [
    ...
    validateRoles(roles) {
        ...
        backgrounds.forEach(bg => {
            // 提取关键专业词汇
            if (bg.includes('技术') || bg.includes('系统') || bg.includes('架构') || bg.includes('工程'))
                uniqueBackgroundKeywords.add('tech');
            if (bg.includes('数据') || bg.includes('分析') || bg.includes('科学'))
                uniqueBackgroundKeywords.add('data');
            if (bg.includes('管理') || bg.includes('经理') || bg.includes('项目'))
                uniqueBackgroundKeywords.add('management');
            if (bg.includes('研究') || bg.includes('学者') || bg.includes('教授'))
                uniqueBackgroundKeywords.add('research');
            if (bg.includes('咨询') || bg.includes('顾问') || bg.includes('策略'))
                uniqueBackgroundKeywords.add('consulting');
            if (bg.includes('创新') || bg.includes('设计') || bg.includes('产品'))
                uniqueBackgroundKeywords.add('innovation');
            if (bg.includes('用户') || bg.includes('体验') || bg.includes('交互'))
                uniqueBackgroundKeywords.add('ux');
            if (bg.includes('跨学科') || bg.includes('综合') || bg.includes('传播'))
                uniqueBackgroundKeywords.add('interdisciplinary');
        });
        // 检查分析角度的多样性
        const uniqueStanceKeywords = new Set();
        stances.forEach(stance => {
            if (stance.includes('系统') || stance.includes('整体') || stance.includes('全局'))
                uniqueStanceKeywords.add('systematic');
            if (stance.includes('创新') || stance.includes('变革') || stance.includes('机会'))
                uniqueStanceKeywords.add('innovative');
            if (stance.includes('跨学科') || stance.includes('融合') || stance.includes('综合'))
                uniqueStanceKeywords.add('interdisciplinary');
            if (stance.includes('实际') || stance.includes('可行') || stance.includes('操作'))
                uniqueStanceKeywords.add('practical');
            if (stance.includes('理论') || stance.includes('学术') || stance.includes('研究'))
                uniqueStanceKeywords.add('theoretical');
            if (stance.includes('用户') || stance.includes('体验') || stance.includes('需求'))
                uniqueStanceKeywords.add('user-focused');
            if (stance.includes('数据') || stance.includes('证据') || stance.includes('分析'))
                uniqueStanceKeywords.add('data-driven');
            if (stance.includes('战略') || stance.includes('长远') || stance.includes('规划'))
                uniqueStanceKeywords.add('strategic');
        });
    ...
    async testConnection() {
        try {
            const testModel = AIModel_1.AIModelFactory.create(this.baseConfig);
    ...
    // 获取角色生成统计信息
    async generateRolesWithStats(topic, roleCount = 3) {
...
exports.ParallelRoleGenerator = ParallelRoleGenerator;
//# sourceMappingURL=ParallelRoleGenerator.js.map