...
import { AIModelFactory } from '../ai/AIModel';

export class AIConnectionFixer {
  
  /**
   * 自动诊断并尝试修复AI连接问题
   */
  static async autoFixConnection(config: AIModelConfig): Promise<{
    success: boolean;
    ...
    try {
      // 1. 基础诊断
      const diagnosis = await AIDiagnosticTool.diagnoseConnection(config);
      ...
      // 3. 最终验证
      if (success) {
        const finalTest = await AIDiagnosticTool.diagnoseConnection(fixedConfig);
   ...
   * 修复Ollama连接问题
   */
  private static async fixOllamaConnection(config: AIModelConfig): Promise<{
    success: boolean;
    ...
    try {
      // 检查可用模型并自动匹配
      const aiModel = AIModelFactory.create(config);
      ...
      // 尝试获取可用模型列表
      try {
        const client = (aiModel as any).client;
        ...
        // 再次验证
        const fixedModel = AIModelFactory.create(fixedConfig);
   ...
   * 修复OpenAI连接问题
   */
  private static async fixOpenAIConnection(config: AIModelConfig): Promise<{
    success: boolean;
    config: AIModelConfig;
    issues: string[];
    fixes: string[];
      ...
      if (!config.baseUrl.startsWith('https://api.openai.com') && 
        ...
        for (const url of possibleUrls) {
          try {
            ...
            const testModel = AIModelFactory.create(testConfig);
   ...
   * 智能匹配最佳模型名称
   */
  private static findBestModelMatch(targetModel: string, availableModels: any[]): string {
    ...
    // 2. 包含匹配
    const containsMatch = modelNames.find(name => 
      name.toLowerCase().includes(target) || target.includes(name.toLowerCase())
    );
   ...
   * 常见模型名称修正映射
   */
  private static getCommonModelNameFixes(): { [key: string]: string } {
    return {
   ...
   * 快速连接测试
   */
  static async quickConnectionTest(config: AIModelConfig): Promise<{
    success: boolean;
    responseTime: number;
    error?: string;
  }> {
