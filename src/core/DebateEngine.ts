import { DebateRole, Statement, DebateSession, DebateConfig } from '../types';
...
export class DebateEngine extends EventEmitter {
  ...
  private isStopped: boolean = false; // 新增：停止标志
  ...
  async startDebate(topic: string, roles: DebateRole[], config: DebateConfig, parentSession?: DebateSession): Promise<DebateSession> {
  ...
  private async conductDebateRound(): Promise<void> {
  ...
  private async conductConsensusRound(): Promise<void> {
    this.emit('debate-status-update', { 
      status: 'converging', 
      round: this.session!.currentRound, 
      message: '寻求共识中...' 
  ...
  private async generateStatement(role: DebateRole, type: Statement['type']): Promise<Statement> {
    // 发送角色开始生成的事件
  ...
  private buildPromptForRole(role: DebateRole, type: Statement['type']): string {
...
- 着重实用性和可操作性`;
  ...
  private buildSystemPromptForRole(role: DebateRole): string {
...
- 要在坚持专业立场和寻求共识之间找到平衡`;
  ...
  private buildContextForRole(role: DebateRole): string {
  ...
  private shouldContinueDebate(): boolean {
  ...
  private async checkForConvergence(): Promise<boolean> {
    if (!this.session || this.session.currentRound < 2) return false;
  ...
  private async generateFinalConsensus(): Promise<void> {
...
4. 确保结论具有可操作性`;
  ...
  private addStatement(statement: Statement): void {
    if (this.session) {
